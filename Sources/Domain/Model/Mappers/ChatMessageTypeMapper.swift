...
private extension ChatMessageTypeMapper {
    ...
    static func handleMessageType(_ entity: Message, textMessage: String?) -> ChatMessageType? {
        ...
        case .timePicker(let content):
            let timeSlots = content.timeSlots.map {
                RichMessageTimeSlot(id: $0.id, duration: $0.duration, startTime: $0.startTime)
            }

            return .richContent(.timePicker(TimePickerItem(title: content.title, sheetTitle: content.sheetTitle, timeSlots: timeSlots)))
        case .unknown:
